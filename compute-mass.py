...
if len(sys.argv)==1:
    outpath="scan-mass-data/result"+".h5"
    inpath="scan-mass-data/input"+".h5"
    min_repeat=3
else:
    outpath="scan-mass-data/result"+sys.argv[1]+".h5"
    inpath="scan-mass-data/input"+sys.argv[1]+".h5"
...
def findmin_success(c):
    ...
    while True:
        ...
        print "max_iter=",max_iteration,"not successful"
...
def checkBFB(c):
    ...
    res=minimize(Vfix, x0, args=(c_nomass,fixed_id,-1),method='Nelder-Mead', tol=1e-7,
                   options={"maxiter":max_iteration0})
